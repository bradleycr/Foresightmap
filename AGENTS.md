...
- **Routes.** `/` map · `/berlin` `/sf` `/global` programming · `/profile` directory · `/calendar` · `/connections` · `/checkin` (and `/checkin/berlin` etc.) · `/stats` (unlisted) · `/polls` (unlisted admin; `/polls/:slug` public vote, `/polls/:slug/live` projector) · `/join?token=…` standing invite (claim existing via roster email, or create Fellow/Grantee/Nodee/Prize Winner). Client-side routing; base-path-aware for GitHub Pages (see `src/utils/router.ts`).
